...
function np() {
       ...
       function designed(obj) {
        id = (obj.id);
				$.ajax({
					type: 'POST',
					url: '/function/ready_blank_2.php?id=' + id + '',
					success: function(data) {
                    id=id.split('|',1);
...
<div class="wrapper">
    <div class="content" style="width: 120%;margin-left: -10%;">
        ...
        <div class="table" style="margin-top: 10px;">
            <div class="head"><h5 class="iFrames" style="width: 96%;">Отгрузка<input type="button" value="Отправить все в бланк №1" class="greenBtn mr10 ml10" style="float: right;" onclick="designed(this)" /></h5></div>
            <table  cellpadding="0" cellspacing="0" border="0" class="display" id="example"  >
				...
				<?php
				    if($_SESSION['access']=='3')
                        {
							$query = "SELECT * FROM `scroll_call` WHERE `status`='16' AND `manager`='".$_SESSION['id']."' ORDER BY `id` DESC";
                            
                        }
                    else
                        {
							$query = "SELECT * FROM `scroll_call` WHERE `status`='16' ORDER BY `id` DESC";
                            
                        }
					$query = "SELECT * FROM call_preorder,scroll_call WHERE call_preorder.call_id=scroll_call.id AND call_preorder.delivery_info LIKE '".$post."%' AND scroll_call.status=16";	
